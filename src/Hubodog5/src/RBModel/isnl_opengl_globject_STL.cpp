...
GLSTL::GLSTL(std::string filename){
	...
	char ff[80];
	int numfacet[2];
    size_t temp_size_dummy;
    temp_size_dummy = fread(ff,sizeof(char),80,binary) ;
    temp_size_dummy = fread(numfacet,sizeof(int),1,binary);
	int num=numfacet[0];
	facet.assign(num,group());
    //printf("%s facecount %d \n",filename.c_str(),num);
    for (int j=0;j<num;j++){
        temp_size_dummy = fread(facet[j].normal,sizeof(float),3,binary);
        temp_size_dummy = fread(facet[j].vertex1,sizeof(float),3,binary);
        temp_size_dummy = fread(facet[j].vertex2,sizeof(float),3,binary);
        temp_size_dummy = fread(facet[j].vertex3,sizeof(float),3,binary);
        temp_size_dummy = fread(facet[j].unuse,sizeof(char),2,binary);
	}

    if(temp_size_dummy !=0){
        ;
    }
