...
LDL_int LDL_cache_numeric	/* returns n if successful, k if D (k,k) is zero */
(
LDL_int n,		/* A and L are n-by-n, where n >= 0 */
LDL_int Ap[],	/* input of size n+1, not modified */
LDL_int Ai[],	/* input of size nz=Ap[n], not modified */
double Ax[],	/* input of size nz=Ap[n], not modified */
LDL_int Ltp[],	/* input of size n+1, not modified */
LDL_int Lti[],	/* input of size lnz=Lp[n], not modified*/
LDL_int Li[],
LDL_int Lp[],
double Lx[],	/* output of size lnz=Lp[n], not defined on input */
double D[],	/* output of size n, not defined on input */
...
LDL_int P[],	/* optional input of size n */
LDL_int Pinv[],	/* optional input of size n */
LDL_int UPattern[] /*Update Pattern of subsequent iterations*/
)
{
	double  l_ki;
	LDL_int i, k, p, kk, jj, tt;
	LDL_int *work;
	LDL_int col_idx;
	LDL_int Sign;
	double delta = 0.0000001;
	double eps = 0.00000000000001;
	work = (LDL_int*)malloc(n*sizeof(LDL_int));
...
LDL_int LDL_row_cache_numeric	/* returns n if successful, k if D (k,k) is zero */
(
LDL_int n,		/* A and L are n-by-n, where n >= 0 */
LDL_int Ap[],	/* input of size n+1, not modified */
LDL_int Ai[],	/* input of size nz=Ap[n], not modified */
...
LDL_int P[],	/* optional input of size n */
LDL_int Pinv[],	/* optional input of size n */
LDL_int UPattern[] /*Update Pattern of subsquent iterations*/
)
{
	double yi, l_ki, Sign, eps, delta;
	delta = 0.0000001;
	eps = 0.00000000000001;
	LDL_int i, k, p, kk, jj;
	for (k = 0; k < n; k++)
